from model_Full_Model import Patient_DA_prediction
...
if __name__ == "__main__":
    ...
    for i, batch in pbar:
        ...
        outputs = model(past_values_da, past_time_features, past_observed_mask, static_categorical_features,
                        future_values_da, future_time_features, text)
        outputs_pred = model.generate(past_values_da, past_time_features, past_observed_mask,
                                      static_categorical_features, future_time_features, text)
        preds.append(outputs_pred['pred'].squeeze())
        targets.append(future_values_da[:, 0])
    preds = torch.cat(preds)
    targets = torch.cat(targets)

    np.save(f"predictions/preds_Full_Model_test.npy", preds.cpu().numpy())
    np.save(f"predictions/targets_Full_Model_test.npy", targets.cpu().numpy())
    torch.save(model, f"models/Full_Model.pt")
