...
class Word2Vec(nn.Module):
    ...
    def get_closest_word(self, batch_word):
        ...
        batch_word = batch_word.repeat(1, 9, 1)
        all_words = torch.arange(9).unsqueeze(0).repeat(batch_word.size(0), 1).to(batch_word.device)
...
class Patient_DA_prediction(nn.Module):
    def __init__(self,config):
        ...
        fusion_dim = 78
        self.da_linear = nn.Linear(da_embedding_dimension, fusion_dim*2)
        ...
        self.da_prediction_head = nn.Linear(4*fusion_dim, 2*fusion_dim)
        self.da_prediction_head2 = nn.Linear(2*fusion_dim, self.config.n_da_client)
...
"Backchannel"]
        ...
        self.text_linear = nn.Linear(768, fusion_dim*2)
    ...
    def forward(self, past_value_da, past_time_features, past_observed_mask, static_categorical_features, future_values_da,future_time_features,text):
        ...
        future_values = self.da_embedding.embeddings_target(future_values_da.long())
        past_values = self.da_embedding.embeddings_target(past_value_da.long())
        ...
        distribution = self.model.output_distribution(outputs.params, loc=outputs.loc, scale=outputs.scale)
        out = torch.mean(distribution.sample_n(100),dim=0).squeeze()
        out = self.da_linear(out)
        concat = torch.cat((out, text_feature), dim=1)
        out = self.da_prediction_head(concat)
        out = self.dropout(out)
        out = F.gelu(out)
        out = self.da_prediction_head2(out)

        loss = outputs.loss + self.loss_function(out, future_values_da.long().squeeze())
    ...
    def generate(self, past_value_da, past_time_features, past_observed_mask, static_categorical_features, future_time_features,text):
        ...
        out = torch.mean(outputs.sequences, dim=1).squeeze()
        out = self.da_linear(out)
        concat = torch.cat((out, text_feature), dim=1)
        out = self.da_prediction_head(concat)
        out = self.dropout(out)
